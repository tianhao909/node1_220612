...
__global__ void naive_histo(int *d_bins, const int *d_in, const int BIN_COUNT) //错误
{
    int myId = threadIdx.x + blockDim.x * blockIdx.x;
    int myItem = d_in[myId];
    int myBin = myItem % BIN_COUNT; //除以分组数， 平均分成16份 
    d_bins[myBin]++; //65536/16 = 4096   读取和写入同时进行， 会出现错误 ； 要进行原子累加； 同时操作， 会进行排队处理
}

__global__ void simple_histo(int *d_bins, const int *d_in, const int BIN_COUNT)//方法二
{
    int myId = threadIdx.x + blockDim.x * blockIdx.x;
    int myItem = d_in[myId];
    int myBin = myItem % BIN_COUNT; //并行化程度低
    atomicAdd(&(d_bins[myBin]), 1); //原子累加， 而不是简单累加 
}
